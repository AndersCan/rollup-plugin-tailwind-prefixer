...
describe("finder", () => {
  ...
  test("can find - class with tailwind utility", () => {
    const actual = finder( ` class="dark:hover:focus:flex"`, ` class="`, `"` );
    expect( actual ).toHaveLength( 1 );
    expect( actual ).toEqual( [
      {
        start: 0,
        end: 30,
        value: ` class="dark:hover:focus:flex"`,
      },
    ] );
  });

  test.skip("can find matches in partial string", () => {
    const actual = finder( ` `, ` absolute">`, `"` );
    expect( actual ).toHaveLength( 1 );
    expect( actual ).toEqual( [
      {
        start: 3,
